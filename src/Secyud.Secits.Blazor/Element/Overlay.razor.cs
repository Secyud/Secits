...
{
    public sealed partial class Overlay : IContentComponent
    {
        /// <summary>
        ...
        /// </summary>
        [CascadingParameter]
        public OverlayContext? OverlayContext { get; set; }
        ...
        [Parameter] public EventCallback OnOverlayMouseUp { get; set; }
        ...
        private string
            _overlayStyle =
                "display: none;"; //initial value prevents from screen flickering when adding overlay to dom; it will be overwritten immediately
        ...
        internal async Task Hide(bool force = false)
        {
            ...
            if (!force && ((!IsContainTrigger(TriggerType.Click) && _mouseInOverlay) || _isPreventHide ||
                           (_childrenToShow.Count > 0) || (Trigger.Visible && !Trigger.VisibleChanged.HasDelegate)))
            {
                return;
            }
        ...
        private async Task AddOverlayToBody(int? overlayLeft = null, int? overlayTop = null)
        {
            ...
            if (Trigger.CheckPlacementChanged() || !_hasAddOverlayToBody)
            {
                ...
                _position = await JsInvokeAsync<OverlayPosition>(
                    JSInteropConstants.OverlayComponentHelper.AddOverlayToContainer,
        ...
        private async Task UpdateParentOverlayState(bool visible)
        {
            ...
            if (!visible)
            {
                if (parentOverlay.IsContainTrigger(TriggerType.Click) ||
                    parentOverlay.IsContainTrigger(TriggerType.ContextMenu))
                {
        ...
        internal async Task UpdatePosition(int? overlayLeft = null, int? overlayTop = null)
        {
            ...
            _position = await JsInvokeAsync<OverlayPosition>(
                JSInteropConstants.OverlayComponentHelper.UpdateOverlayPosition,
                Ref.Id, Ref, Trigger.Ref, Trigger.PlacementWithRTL, Trigger.PopupContainerSelector,
                Trigger.BoundaryAdjustMode, triggerIsWrappedInDiv, Trigger.PrefixCls,
                VerticalOffset, HorizontalOffset, ArrowPointAtCenter, overlayTop, overlayLeft);
